...
import {
  ...
  handleRouteError,
  httpStatusCodes,
  extractDbQueryParams,
} from './routeUtils';
import {
  quizAttemptController,
  quizQuestionResultController,
  quizController,
  questionController,
} from '../dbControllers';
...
import QuizQuestionResult, {
  PopulatedQuizQuestionResultType,
} from '../../db/models/QuizQuestionResult';
...
export const quizAttemptRouteController: IQuizAttemptRouteController = {
  getAll: async (
    req: Request,
    res: Response,
  ): Promise<IApiResponse<QuizAttemptModelResponse[]>> => {
    try {
  ...
  getById: async (req: Request, res: Response): Promise<IApiResponse<QuizAttemptModelResponse>> => {
    try {
      const quizAttempt = await quizAttemptController.getById(
        req.params.id,
  ...
  updateById: async (
    ...
    try {
      const quizAttempt = await quizAttemptController.updateById(
        req.params.id,
        req.body,
        extractDbQueryParams(req),
      );
  ...
  deleteById: async (
    ...
    try {
      const quizAttempt = await quizAttemptController.deleteById(
        req.params.id,
        extractDbQueryParams(req),
      );
      if (!quizAttempt) throw new Error('Quiz attempt not found.');
  ...
  getByQuizId: async (
    ...
    try {
      const quizAttempts = await quizAttemptController.getByQuizId(
        quizId,
        extractDbQueryParams(req),
      );
      return res.status(httpStatusCodes.OK).json({data: quizAttempts});
  ...
  addAnsweredQuestion: async (
    ...
    try {
      ...
      const quizAttempt: QuizAttemptModelResponse | null = await quizAttemptController.getById(
        id,
        queryParams,
      );
      if (!quizAttempt) throw new Error('Quiz attempt not found.');
      ...
      for (const question of quizAttempt.answeredQuestions as QuizQuestionResultType[]) {
        if (questionId === (question?.question?._id?.toString() ?? question))
      ...
      const updatedQuizAttempt = await quizAttemptController.updateById(
        id,
        {answeredQuestions: [questionResult._id]},
        queryParams,
      );
  ...
  gradeQuizAttempt: async (
    req: Request,
    res: Response,
  ): Promise<IApiResponse<PopulatedQuizAttemptType>> => {
    const {id} = req.params;
    const queryParams = extractDbQueryParams(req);

    try {
      // get the quizAttempt
      const ungradedAttempt: QuizAttemptModelResponse | null = await quizAttemptController.getById(
        id,
        {...queryParams, needToPopulate: true},
      );
      if (!ungradedAttempt) throw new Error('Quiz attempt not found.');
      // get the quiz
      const originalQuizData: QuizModelResponse | null = await quizController.getById(
        ungradedAttempt.quizId.toString(),
        queryParams,
      );
      if (!originalQuizData) throw new Error('Quiz not found.');

      // check for answered questions, if there are none, throw an error
      if (!ungradedAttempt.answeredQuestions?.length)
        throw new Error('Quiz attempt has no answered questions.');
      ...
      const answeredQuestions =
        ungradedAttempt.answeredQuestions as unknown as PopulatedQuizQuestionResultType[];
      ...
      for (const answeredQuestion of answeredQuestions) {
        ...
        if (
          ...
          const userSelections = answeredQuestion.selectedAnswer
            .split(',')
            .map((selection: string) => selection.trim().toLowerCase().replace(' - ', '-'))
          ...
          const foundDelim = aiProvidedDelims.find((delim: string) =>
            correctSelections[0].includes(delim),
          );
          ...
          const selectedAnswers = answeredQuestion.selectedAnswer
            .split(',')
            .filter(str => str.length > 0)
      ...
      const numberOfCorrectAnswers = ungradedAttempt.answeredQuestions.filter(
      ...
      const gradedAttempt = await quizAttemptController.updateById(
        id,
        {
